...
import { Activity, Calendar, ShoppingBag, Sparkles, TrendingUp, Zap } from 'lucide-react';
  ...
  return (
    <PageWrapper>
       <div className="flex flex-col md:flex-row md:items-center justify-between gap-8 mb-16 font-sans">
          <div>
             <h2 className="text-5xl md:text-6xl font-serif text-brand-green italic uppercase tracking-tighter shadow-brand-green/20">REVENUE OPS</h2>
             <p className="text-white/40 font-black text-xs md:text-sm uppercase tracking-[0.3em] mt-2">DEEP ANALYTICS / HEALTHY PERFORMANCE PROTOCOL</p>
          </div>
          <div className="grid grid-cols-2 lg:flex gap-4">
             <div className="bg-white/5 p-6 rounded-sm shadow-2xl border border-white/10 text-center px-4 md:px-10 ring-1 ring-white/5 relative overflow-hidden">
                <div className="absolute top-0 right-0 p-1 opacity-10"><Activity className="w-4 h-4 text-brand-green" /></div>
                <p className="text-[10px] font-black text-white/40 uppercase tracking-widest mb-1">DAILY REVENUE</p>
                <p className="text-2xl md:text-3xl font-serif text-brand-green italic neon-text uppercase">4.2MĐ</p>
             </div>
             <div className="bg-white/5 p-6 rounded-sm shadow-2xl border border-white/10 text-center px-4 md:px-10 ring-1 ring-white/5 relative overflow-hidden">
                <div className="absolute top-0 right-0 p-1 opacity-10"><ShoppingBag className="w-4 h-4 text-brand-orange" /></div>
                <p className="text-[10px] font-black text-white/40 uppercase tracking-widest mb-1">MONTHLY UNITS</p>
                <p className="text-2xl md:text-3xl font-serif text-brand-orange italic uppercase tracking-tighter">342</p>
             </div>
          </div>
       </div>
                      ...
                      {(['day', 'week', 'month'] as const).map((range) => (
                        <button
                          ...
                          className={cn(
                            ...
                            timeRange === range ? "bg-brand-green text-black shadow-md neon-glow" : "text-white/40 hover:text-white"
                          )}
                        >
                   ...
                   <ResponsiveContainer width="100%" height="100%">
                      {timeRange === 'day' ? (
                        <AreaChart data={analytics?.dayData}>
                           ...
                           <XAxis dataKey="date" axisLine={false} tickLine={false} tick={{ fontSize: 10, fill: 'rgba(255,255,255,0.3)', fontWeight: 'bold' }} dy={10} />
                           <YAxis hide />
                           <Tooltip contentStyle={{ backgroundColor: '#1A1A1A', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '4px', color: '#fff' }} />
                           <Area type="monotone" dataKey="revenue" stroke="#CCFF00" strokeWidth={4} fillOpacity={1} fill="url(#colorRev)" />
                        </AreaChart>
                      ) : (
                        <BarChart data={analytics?.dayData}>
                           ...
                           <XAxis dataKey="date" axisLine={false} tickLine={false} tick={{ fontSize: 10, fill: 'rgba(255,255,255,0.3)', fontWeight: 'bold' }} dy={10} />
                           <YAxis hide />
                           <Tooltip cursor={{ fill: 'rgba(255,255,255,0.05)' }} contentStyle={{ backgroundColor: '#1A1A1A', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '4px', color: '#fff' }} />
                           <Bar dataKey="revenue" fill="#CCFF00" radius={[2, 2, 0, 0]} barSize={timeRange === 'week' ? 50 : 20}>
                              {analytics?.dayData?.map((entry: any, index: number) => (
                                 <Cell key={index} fill={index === (analytics?.dayData?.length - 1) ? '#e07a5f' : '#CCFF00'} />
                              ))}
                           </Bar>
                        </BarChart>
                      )}
             ...
             <div className="grid md:grid-cols-2 gap-10">
                <div className="paper-texture p-8 rounded-sm shadow-2xl border-t-8 border-brand-orange ring-1 ring-white/10">
                   <h3 className="font-serif text-2xl mb-8 italic flex items-center gap-3 uppercase tracking-tight text-white">
                      <Calendar className="w-6 h-6 text-brand-orange" /> LOAD FREQUENCY
                   </h3>
                   <div className="h-[200px]">
                      <ResponsiveContainer width="100%" height="100%">
                         <LineChart data={analytics?.dayData}>
                            <XAxis dataKey="date" hide />
                            <Tooltip contentStyle={{ backgroundColor: '#1A1A1A', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '4px' }} />
                            <Line type="monotone" dataKey="orders" stroke="#e07a5f" strokeWidth={5} dot={{ r: 6, fill: '#e07a5f', strokeWidth: 2, stroke: '#1A1A1A' }} />
                         </LineChart>
                      </ResponsiveContainer>
                   </div>
                </div>
                <div className="paper-texture p-8 rounded-sm shadow-2xl flex flex-col justify-center items-center text-center ring-1 ring-white/10 relative overflow-hidden">
                   ...
                   <h3 className="font-serif text-3xl mb-3 italic uppercase tracking-tighter text-brand-green">SMART TACTIC</h3>
                   <p className="text-xs text-white/50 leading-relaxed font-bold uppercase tracking-widest px-4">WEDNESDAY LUNCH PEAK DETECTED. EXECUTE "MID-WEEK BOOST" PROTOCOL TO MAXIMIZE LOAD CAPACITY.</p>
                </div>
             </div>
          </div>
                   ...
                   <div key={idx} className="flex items-center gap-6 group">
                      ...
                      <div className="flex-1">
                         <div className="flex justify-between items-end mb-2">
                            <p className="text-sm font-black text-white uppercase tracking-widest">{dish.name}</p>
                            <span className="text-[10px] font-black text-brand-green tracking-[0.2em]">{dish.sales} UNITS</span>
                         </div>
